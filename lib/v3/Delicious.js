...
function startSynchronizationProcess() {
	...
	console.log('TODO sendEvent syncProcessStarted to update the View');
...
function syncBundlesAsync() {
	deliciousRequest(DELICIOUS_BUNDLES_URL,		
		...
		function(response, status) {
			...
			if (response) console.log(" responseText:"+response.responseText);
...
function triggerSyncSuccess() {
	...
	console.log('TODO sendEvent onSyncSuccess to update the View');
