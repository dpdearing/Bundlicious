...
try {
	chrome.extension.onConnect.addListener(function(port) {
		console.log('View got onConnect:"'+port+'"');
		try {
			port.onMessage.addListener(function(msgObj) {				
				console.log('View got a message: ' + msgObj.msg);
				...
				switch (msgObj.msg) {
					...
					case 'syncProcessStarted':
						console.log("syncProcessStarted - updating View");
						displayProcess('Synchronizing bookmarks...');
					break;
					
					case 'updateOptionsWindow':
						console.log("updateOptionsWindow - (next) restorePreferences ");
						restorePreferences();
					break;
				}
			});
