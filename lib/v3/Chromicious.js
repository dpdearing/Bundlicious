function Chromicious() {
	...
	try {
		chrome.extension.onConnect.addListener(function(port) {
			console.debug('Chromicious got onConnect');
			try {
				port.onMessage.addListener(function(msgObj) {
					console.debug('Chromicious got a message: ' + msgObj.msg);
	...
	this.sendEvent = function(eventName, params) {
		try {
			console.log("trying to send event ["+eventName+"], likely to FAIL...");
...
Chromicious.getInstance = function() {
    ...
    for (var i = 0; i < views.length; i++) {
        if (views[i].location.href == chrome.extension.getURL(BACKGROUND_URL)) {
